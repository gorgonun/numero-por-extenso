...
sys.path.append(os.path.join(HERE, "../libs/util/"))
import number_in_word
...
def test_correct_ten_unity_to_word():
    ...
    for number in numbers:
        print(number_in_word.ten_unity_to_word(number))
        i += 1
...
def test_correct_hundred_to_word():
    ...
    i = 0
    for number in numbers:
        assert number_in_word.hundred_to_word(*number) == word[i]
        i += 1
...
def test_check_concat():
    ...
    i = 0
    for number in numbers:
        number_in_word.check_concat(number) == result[i]
        i += 1
...
def test_correct_thousand_to_word():
    ...
    i = 0
    for number in numbers:
        assert number_in_word.thousand_to_word(*number) == word[i]
        i += 1

test_correct_thousand_to_word()